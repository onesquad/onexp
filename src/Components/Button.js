...
const Button = ({
  type, to, text, icon, iconPos, iconSize, iconColor, disabled, hasUnderline,
  loading, fullWidth, loaderType, loadingText, extraClass, inheritFontSize,
  onClick = noop,
  ...otherProps
}) => {
  ...
  const getInnerContent = () => {
    if (loading && !isIconButton) {
     return <React.Fragment>{text}<ButtonLoader/></React.Fragment>;
    } else if (icon) {
      const iconProps = {
        icon,
      ...
      return <React.Fragment><Icon {...iconProps} className={classnames({ 'mr--sm': !isIconButton })}/>{text}</React.Fragment>;
    } else {
      return text
    }
  }
